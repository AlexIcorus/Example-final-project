...
from Status.status_form_controller_ui import Ui_frm_status
from Status.status_controller import statusController
from Status.status_form_popup import statusFormPopUp


class FrmstatusController(QWidget):
    
    def __init__(self):
        super().__init__()
        self.ui = Ui_frm_status()
        self.statusformpopup = statusFormPopUp()
        self.ui.setupUi(self)
        self.devicecontroller = statusController()
    ...
    def initUi(self):
        ...
        self.btn_insert_popup = self.statusformpopup.ui.btn_insert
        self.btn_edit_popup = self.statusformpopup.ui.btn_edit
        self.btn_cancel_popup = self.statusformpopup.ui.btn_cancel
        self.btn_clear_popup = self.statusformpopup.ui.btn_clear
        ...
        self.lbl_status_id = self.ui.lbl_did
        self.lbl_id_popup = self.statusformpopup.ui.lbl_did_
        self.txt_status_name = self.statusformpopup.ui.txt_status_name
        self.txt_remark = self.statusformpopup.ui.txt_remark
        self.txt_search = self.statusformpopup.ui.txt_search
    ...
    def clearPopUp(self):
        ...
        self.txt_status_name.clear()
        self.txt_status_name.setFocus()

    def closePopUp(self):
        self.statusformpopup.close()
        self.clearPopUp()
    ...
    def loadAndShowData(self):
        ...
        try:
            result = self.devicecontroller.selectStatusData()  # Ensure this method exists and fetches data
    ...
    def hideEditButton(self):
        ...
        self.lbl_status_id.setHidden(True)
    ...
    def openInsertPopUpFormDevice(self):
        self.statusformpopup.show()
        self.statusformpopup.activateWindow()
        self.txt_status_name.setFocus()
    ...
    def selectRecord(self):
        ...
        if select_row != -1:
            ...
            status_id = self.tbshow.item(select_row, 0).text().strip()
            status_name = self.tbshow.item(select_row, 1).text().strip()
            remark = self.tbshow.item(select_row, 2).text().strip()
        

            # Update the labels and combo boxes with the selected row data
            self.lbl_status_id.setText(status_id)
            self.lbl_id_popup.setText(status_id)
            self.txt_status_name.setText(status_name)
    ...
    def add_info(self):
        try:
            ...
            device_info = self.getSatusInfo()

            if device_info is None:
                raise ValueError("No device info returned")

            
            if not device_info["status_name"].strip():
            ...
            # Check if all IDs are valid
                    ...
                    self.devicecontroller.add_info(
                        status_name=device_info["status_name"], 
        ...
        try:
            ...
            if selected_option == QMessageBox.StandardButton.Yes:
                ...
                if update_result:
                    ...
                    QMessageBox.information(self, "Success", "status information updated successfully.",
                                            QMessageBox.StandardButton.Ok)
            else:
                self.openEditPopUpFormDevice()
    ...
    def delete_info(self):
        try:
            ...
            if selected_option == QMessageBox.StandardButton.Yes:
                delete_result = self.devicecontroller.deleteData(status_id=device_info["status_id"])
    ...
    def getSatusInfo(self):
        status_id = self.lbl_status_id.text().strip()
        status_name = self.txt_status_name.text().strip()
